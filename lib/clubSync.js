...
// MEMBER-AWARE GAME BUDGET: every game costs (2 + members) Steam calls,
// so a fixed games-per-run budget silently blows the 60s serverless
// ceiling as the club grows (the old cron 60 broke around 15 members).
// Instead, each writer names its CALL budget — chosen for ~45s of work
// at concurrency 5 with retry headroom — and the game count follows.
export function gameBudget(memberCount, { callBudget, min = 10, max = 75 }) {
  const m = Math.max(1, memberCount);
  return Math.max(min, Math.min(max, Math.floor((callBudget - m - 1) / (2 + m))));
}
...
function mergeProfiles(prevProfiles, fetched, nowEpoch, clubIds) {
  const prev = prevProfiles ?? {};
  const meta = fetched.profileMeta ?? {};
  const inClub = (appid) => clubIds.has(Number(appid));
  ...
  for (const [sid, fp] of Object.entries(fetched.profiles ?? {})) {
    ...
    if (!ownedFetched) {
      out[sid] = p
        ...
        : { persona, avatar, playtime: {}, lastPlayed: {}, owned: [], ownedAt: 0, ownedStrikes: {} };
      ownedCarried.push(sid);
      continue;
    }
    // PAYLOAD DIET: GetOwnedGames returns the member's ENTIRE Steam
    // library, and the payload used to store minute-maps for all of
    // it — scaling with friends' personal hoards, not the catalog.
    // Now the minute maps (playtime/lastPlayed) keep CLUB games only;
    // full-library ownership survives as a compact sorted appid array
    // (`owned`) for features that check ownership beyond the catalog
    // (Century's dusty covers). Pre-diet foreign entries drain out of
    // old payloads here, strike-free, on each member's next good fetch.
    const ownedArrFetched = Object.keys(fp.playtime ?? {}).map(Number).sort((a, b) => a - b);
    // Shrink guard, same spirit as the rated-wipe veto: a "successful"
    // list under half the previous size smells like a partial answer —
    // keep the previous array. Ordinary shrinks (refunds) pass clean.
    const owned = (p?.owned?.length ?? 0) > 0 && ownedArrFetched.length * 2 < p.owned.length
      ? p.owned : ownedArrFetched;
    const playtime = {};
    const lastPlayed = {};
    for (const [appid, mins] of Object.entries(fp.playtime ?? {})) {
      if (!inClub(appid)) continue;
      playtime[appid] = mins;
      if (fp.lastPlayed?.[appid] !== undefined) lastPlayed[appid] = fp.lastPlayed[appid];
    }
    const ownedStrikes = {};
    for (const [appid, mins] of Object.entries(p?.playtime ?? {})) {
      if (!inClub(appid)) continue;                      // foreign leftovers from pre-diet payloads: drained
    ...
    out[sid] = { persona, avatar, playtime, lastPlayed, owned, ownedAt: nowEpoch, ownedStrikes };
  }
...
function patchPlayerHoles(fetchedGames, playerMisses, prevPayload) {
  if (!prevPayload) return { games: fetchedGames, playersCarried: 0, gamesVetoed: 0 };
  const misses = playerMisses ?? {};
  const prevById = new Map((prevPayload.games ?? []).map((g) => [String(g.appid), g]));
  let playersCarried = 0, gamesVetoed = 0;
  const rated = (x) => (x?.ach ?? []).filter((a) => a.pct > 0).length;
  const games = fetchedGames.map((g) => {
    const prevG = prevById.get(String(g.appid));
    if (!prevG) return g;
    // 3. RATED-WIPE VETO: an established game (cache has rated
    //    achievements) arriving with ZERO rated achievements is the
    //    husk signature — a transiently-empty percentages answer
    //    routed through the new-title branch. Content is vetoed, the
    //    cached entry rides through; the fetch stamp still advances
    //    (the game stays in the fetched list), so nothing loops.
    //    Genuinely new games have no rated cache and pass untouched.
    if (rated(g) === 0 && rated(prevG) > 0) { gamesVetoed++; return prevG; }
  ...
  return { games, playersCarried, gamesVetoed };
}

export function mergePayload(prevPayload, fetched, clubIds, nowEpoch) {
  const { games: fetchedGames, playersCarried, gamesVetoed } = patchPlayerHoles(fetched.games, fetched.playerMisses, prevPayload);
  ...
  const { profiles, ownedCarried } = mergeProfiles(prevPayload?.profiles, fetched, nowEpoch, clubIds);
  return {
    payload: {
      games: [
        ...fetchedGames,
    ...
    gotIds,
    carried: { owned: ownedCarried, players: playersCarried, gamesVetoed },
  };
}
