...
// Freshly-fetched games replace their cache entries wholesale — so when
// one player's GetPlayerAchievements call hard-failed inside an
// otherwise-good game fetch, that player would silently vanish from the
// game (perfect count flickers 10 → 9 → 10). Patch the hole with the
// player's previous unlocks for that game before the replacement lands.
function patchPlayerHoles(fetchedGames, playerMisses, prevPayload) {
  const misses = playerMisses ?? {};
  if (!prevPayload || !Object.keys(misses).length) return { games: fetchedGames, playersCarried: 0 };
  const prevById = new Map((prevPayload.games ?? []).map((g) => [String(g.appid), g]));
  let playersCarried = 0;
  const games = fetchedGames.map((g) => {
    const missed = misses[String(g.appid)] ?? [];
    const prevG = prevById.get(String(g.appid));
    if (!missed.length || !prevG) return g;
    const players = { ...g.players };
    for (const sid of missed) {
