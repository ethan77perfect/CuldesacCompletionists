...
async function steamJSON(url, retries = 2, envelope = null) {
  // An answer must be RECOGNIZABLE, not merely parseable: each Steam
  // endpoint wraps real answers in a known envelope key. Throttle
  // refusals sometimes arrive as JSON too — an empty {} — and one of
  // those impersonating an answer on the global call, with schema
  // still serving from CDN, once emitted an established game as an
  // all-Unrated, zero-player husk (the Scritchy Scratchy wipe). No
  // envelope, no answer — any status code, 200 included.
  const answered = (body) => body && typeof body === "object" && (!envelope || envelope in body);
    ...
    try {
      ...
      if (r.ok || r.status === 403 || r.status === 400) {
        try {
          ...
          if (answered(body)) return { data: body, hardMiss: false };
        } catch { /* unparseable — not an answer */ }
        // parseable-but-unrecognizable falls through to retry too
      }
...
// The envelope each endpoint wraps a real answer in.
const ENVELOPE = { summary: "response", owned: "response", global: "achievementpercentages", schema: "game", player: "playerstats" };
...
export async function fetchClubData(key, steamids, appids, { withProfiles = true, concurrency = 5 } = {}) {
  ...
  const outcomes = await pool(tasks, concurrency, (t) => steamJSON(t.url, 2, ENVELOPE[t.kind]));
