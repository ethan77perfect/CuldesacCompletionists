...
export async function fetchClubData(key, steamids, appids, { withProfiles = true, concurrency = 5 } = {}) {
  ...
  tasks.forEach((t, i) => {
    ...
    if (t.kind === "global") { slot.global = r; slot.globalMiss = outcomes[i].hardMiss; }
    else if (t.kind === "schema") { slot.schema = r; slot.schemaMiss = outcomes[i].hardMiss; }
    else {
      slot.players[t.sid] = r;
      if (outcomes[i].hardMiss) slot.playerMiss.push(t.sid);
    }
  });
