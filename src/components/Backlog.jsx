  ...
  return (
    <div style={{ display: "grid", gap: 14, maxWidth: 640 }}>
      <div style={S.panel}>
        ...
        <div style={{ marginTop: 12, display: "flex", gap: 8, alignItems: "center", fontSize: 13, color: "var(--muted)" }}>
          Voting as
        ...
        {sorted.map((b) => {
          ...
          return (
            <div key={b.appid} style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 12, flexWrap: "wrap" }}>
              <button style={{ ...S.btnGhost, ...(voted ? { color: "var(--accent)", borderColor: "var(--accent-border)" } : {}) }}
                disabled={busy}
              ...
              <span style={{ flex: 1, minWidth: 140, fontSize: 14, fontWeight: 600 }}>
                {b.name}
                {proposer && <span style={{ fontSize: 12, color: "var(--muted)", fontWeight: 400 }}> · proposed by <span style={{ color: proposer.color }}>{proposer.name}</span></span>}
              </span>
